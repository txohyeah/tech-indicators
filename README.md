...
| `ignition` | 起爆点择时：`CROSS(RSI6,40)` 信号 + 三道买入过滤（跌够深/已离底/起爆K线形态干净）+ 完整卖出状态机（结构止损、新起爆点重锚、浮盈 20% 后按回撤 25% 移动止盈、金牛上沿压制减半或熊市通道清仓）。三道条件分开暴露，默认组成非唯一合理搭配（见模块 docstring 与 memory 记录）。注意 `golden_channel_state` 的卖出端判据受上述未来函数影响，回测务必传 `causal=True` |
